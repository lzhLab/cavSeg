...
class FPN(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes, back_bone='resnet101', pretrained=True):
        ...
        self.back_bone = ResNet50(pretrained=False)
        ...
        self.dconv_st = double_conv(3, 64)
        self.dconv_up1 = double_conv(64, 32)
        self.dconv_up2 = double_conv(32, 3)
    ...
    def forward(self, x):
        ...
        return sur,self._downsample(self.conv3(s2 + s3 + s4 + s5))
'''
...
if __name__ == "__main__":
    model = FPN([3,4,5,3], 1, back_bone="resnet50")
    input = torch.rand(1,3,64,64)
    sur,output = model(input)
    print(output.shape)#(1,1,512,512)
    print(sur.shape)
