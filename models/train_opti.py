...
from models_opti import VesselSegNet
...
def train_epoch(epoch, model, dl, optimizer, criterion, criterion2):
    ...
    for pt, x2, mask in bar:
        ...
        for i in range(l):
            loss1 += structure_loss(stage_list[i], mask).to(device)

        loss2 = criterion2(output, mask).to(device)
        #loss2 = structure_loss(output, mask).to(device)
...
def val_epoch(model, dl, criterion):
    ...
    for pt, x2, mask in dl:
        ...
        loss_v += structure_loss(output, mask).item()
        dice_v += dice_metric(output, mask)
        voe_v += voe_metric(output, mask)
        rvd_v += rvd_metric(output, mask)
        acc_v += acc_m(output, mask)
        sen_v += sen_m(output, mask)
        spe_v += spe_m(output, mask)
